...
var NA = function() {
  ...
  this.initialize = function(accountId, domain, callback) {
  ...
  this.trackPage = function (title, page, utmhid, callback) {
    ...
    if(!callback) {
      this.runQuery(pageViewParams, function(error, response) {
        self.emit('viewed', error, response);
      });
      return self;
    } else {
  ...
  this.trackEvent = function (category, action, label, value, utmni, utmhid, callback) {
    ...
    if(!callback) {
      this.runQuery(eventParams, function(error, response) {
        self.emit('tracked', error, response);
      });
      return self;
    } else {
...
util.inherits(NA, EventEmitter);
module.exports = new NA();
